...
def getImagesToCategorize(cursor):
	...
	try:
		...
		result.append((lastimage, list(set(gals)), list(set(cats))))
		lastimage= ''
		gals = []
		cats = []
		# Start over
...
def main():
    ...
    for (image, gals, cats) in getImagesToCategorize(cursor):
