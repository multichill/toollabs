...
# -*- coding: utf-8  -*-
'''
Simple script to create the daily subcategory of http://commons.wikimedia.org/wiki/Category:OTRS_received
...
def createCategory (date = None):
    ...
    page = wikipedia.Page(wikipedia.getSite(u'commons', u'commons'), u'Category:OTRS received as of ' + str(int(day)) + u' ' + month + u' ' + year)
    wikipedia.output(u'Working on ' + page.title())
    if not page.exists():
	toPut = u'{{OTRS received header|day=' + day + u'|month=' + month + u'|year=' + year + u'}}' 
