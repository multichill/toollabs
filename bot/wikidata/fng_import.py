    ...
    def run(self):
        ...
        for painting in self.generator:
            ...
            if painting[u'id'] in self.paintingIds:
                ...
                if painting.get(u'entitle'):
                    data['labels']['en'] = {'language': 'en', 'value': painting[u'entitle']}
                if painting.get(u'fititle'):
                    data['labels']['fi'] = {'language': 'fi', 'value': painting[u'fititle']}
                if painting.get(u'svtitle'):
                    data['labels']['sv'] = {'language': 'sv', 'value': painting[u'svtitle']}
...
def getPaintingGenerator(query=u''):
    ...
    * Grab data from paintings
    ...
    Europe = 5, 0 - 162 = 1451
    Americas = 3, 0 - 240 = 2146
    Contemporary = 4, 0 - 77 = 678
    
    '''

    filename = u'c:/TEMP/fng-data-dc.json'
    jsonfile = open(filename, u'r')
    #for line in jsonfile:
    #    print line
    jsondata = json.load(jsonfile)
    for paintingMetadata in jsondata.get('descriptionSet'):
        metadata = {}
        metadata['collection'] = u'Q2983474'
        ...
        if foundpainting:
            ...
            if paintingMetadata.get('title'):
                for titlefield in paintingMetadata.get('title'):
                    if titlefield.get(u'en'):
                        metadata[u'entitle'] = titlefield.get(u'en')
                    elif titlefield.get(u'se'):
                        metadata[u'svtitle'] = titlefield.get(u'se')
                    elif titlefield.get(u'fi'):
                        metadata[u'fititle'] = titlefield.get(u'fi')
            else:
                metadata[u'entitle'] = u'<no title given>'
            ...
            if givenname:
                metadata['creator'] = u'%s %s' % (givenname.strip(), surname.strip())
            else:
    ...
    baseurl = u'http://www.mfa.org/collections/search?search_api_views_fulltext=&sort=search_api_aggregation_1&order=asc&f[0]=field_classifications%%3A16&f[1]=field_collections%%3A3&page=%s' 
...
def main():
    ...
    paintingsBot = PaintingsBot(paintingGen, 217, u'2983474')
    paintingsBot.run()
