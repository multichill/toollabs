...
# -*- coding: utf-8 -*-
"""
Bot to scrape paintings from the Los Angeles County Museum of Art website.

http://collections.lacma.org/search/site?page=7&f[1]=im_field_classification%3A22
...
def getPaintingGenerator(query=u''):
    ...
    * Loop over http://collections.lacma.org/search/site?f[1]=im_field_classification%3A22&page=1 - 112 and grab paintings
    * Grab data from paintings
    ...
    baseurl = u'http://collections.lacma.org/search/site?f[1]=im_field_classification%%3A22&page=%s' 
    ...
    for i in range(111,113):
        ...
        itemregex = u'<div class="search-result-data"><a href="/(node/\d+)">'

        for match in re.finditer(itemregex, searchData, flags=re.M):
            n = n + 1
            metadata = {}
            metadata['collection'] = u'Q1641836'
            metadata['collectionshort'] = u'LACMA'
            metadata['location'] = u'Q1641836'
            
            url = u'http://collections.lacma.org/%s' % (match.group(1),) 
            metadata['url'] = url
            ...
            artistregex = u'<div class="field artist-name"><h2><div><a href="/node/(\d+)">([^\(]+)\s\(([^\)]+)\)</a>'
            aristmatch = re.search(artistregex, itemData)

            if aristmatch:
                metadata['creator'] = htmlparser.unescape(unicode(aristmatch.group(2), "utf-8")).strip()
            else:
                metadata['creator']=u'anonymous'
            idregex = u'( in\.|64\)|cm\)| cm\)( each| irregular)?)\s*</div><div class="field">(?P<credit>[^<]+)\s\((?P<id>[^\)]+)\)</div>'
            idmatch = re.search(idregex, itemData, re.M)
            if not idmatch:
                idregex = u'\((?P<id>M\.\d+\.\d+)\)</div>'
                idmatch = re.search(idregex, itemData, re.M)
            
            if idmatch:
                metadata['id'] = htmlparser.unescape(unicode(idmatch.group('id'), "utf-8")).strip()
            else:
                print u'Failed, adding url to list'
                fails.append(url)
                continue
            ...
            """
            yield metadata
    print u'Number of fails: %s' % (len(fails),)
    print fails
...
def main():
    ...
    paintingsBot = PaintingsBot(paintingGen, 217, u'1641836')
    paintingsBot.run()
