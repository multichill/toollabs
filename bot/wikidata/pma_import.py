...
# -*- coding: utf-8 -*-
"""
Bot to scrape paintings from the Philadelphia Museum of Art website.

http://www.pafa.org/museum/The-Collection-Greenfield-American-Art-Resource/Tour-the-Collection/Advanced-Search/Search-Results/1068/medium--Painting/
...
def getPaintingGenerator(query=u''):
    ...
    * Loop over http://www.philamuseum.org/collections/results.html?results=54&searchTxt=&searchNameID=&searchClassID=3&provenance=0&audio=0&onView=0&searchOrigin=&searchDeptID=&action=post&page=1 - 66 and grab paintings
    * Grab data from paintings 1-86

    searchDeptID=5 - European paintings - 2027 - 1-38
    searchDeptID=2 - American Art - 341 - 1-7
    searchDeptID=47 - Contemporary Art - 260 - 1-5
    searchDeptID=7 - Modern Art - 636 1- 12
    searchDeptID=1 - Prints, Drawings, and Photographs 1 - 2
    
    '''

    

    baseurl = u'http://www.philamuseum.org/collections/results.html?results=54&searchTxt=&searchNameID=&searchClassID=3&provenance=0&audio=0&onView=0&searchOrigin=&searchDeptID=1&action=post&page=%s' 
    ...
    for i in range(1,2):
        ...
        # <a class="thumbnailTMS"  href="/collections/permanent/88332.html

        itemregex = u'<a class="thumbnailTMS"\s*href="(/collections/permanent/\d+)\.html\?'

        for match in re.finditer(itemregex, searchData, flags=re.M):
            n = n + 1
            metadata = {}
            metadata['collection'] = u'Q510324'
            metadata['collectionshort'] = u'PMA'
            metadata['location'] = u'Q510324'
            

            # No ssl, faster?
            url = u'http://www.philamuseum.org%s.html' % (match.group(1),) 
            metadata['url'] = url
            ...
            titleregex = u'<meta name="title" content="([^"]+)"\s*/>'
            ...
            descriptionregex = u'<meta name="description" content="([^"]+)"\s*/>'
            descriptionmatch = re.search(descriptionregex, itemData)
            temptitle = metadata['title'].replace(u'(', u'').replace(u')', u'')
            metadata['creator'] = htmlparser.unescape(unicode(descriptionmatch.group(1), "utf-8")).replace(temptitle, u'').strip()
            if metadata['creator']==u'':
                metadata['creator']=u'anonymous'

            dateregex = u'<b>Date:</b><br/>([^<]+)<p></p>'
            datematch = re.search(dateregex, itemData)
            metadata['date'] = htmlparser.unescape(unicode(datematch.group(1), "utf-8")).strip()

            mediumregex = u'<b>Medium:</b><br/>([^<]+)<p></p>'
            mediummatch = re.search(mediumregex, itemData)
            metadata['medium'] = htmlparser.unescape(unicode(mediummatch.group(1), "utf-8")).strip()

            idregex = u'</p><b>Accession Number:</b><br/>([^<]+)<p></p>'
...
def main():
    ...
    paintingsBot = PaintingsBot(paintingGen, 217, u'510324')
    paintingsBot.run()
