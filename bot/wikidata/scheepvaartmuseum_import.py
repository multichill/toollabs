...
# -*- coding: utf-8 -*-
"""
Bot to import paintings from the Het Scheepvaartmuseum in Amsterdam to Wikidata.

The have Adlib/Axiel running at https://collectie.hetscheepvaartmuseum.nl/results
...
def get_scheepvaartmuseum_generator():
    """
    Generator to return Museum Gouda paintings
    """
    start_url = 'https://collectie.hetscheepvaartmuseum.nl/search/detail?database=collect&fieldname=Field_Objectname&value=schilderij'

    # TODO: Find missing ones
    session = requests.Session()
    session.get(start_url)

    base_search_url = 'https://collectie.hetscheepvaartmuseum.nl/resultsnavigate/%s'
    ...
    for i in range(1, 48):
        ...
        work_url_regex = '<a title="[^"]+" href="https://collectie.hetscheepvaartmuseum.nl/Details/collect/(\d+)">'
        matches = re.finditer(work_url_regex, search_page.text)

        for match in matches:
            metadata = {}
            url = 'https://collectie.hetscheepvaartmuseum.nl/Details/collect/%s' % (match.group(1),)

            item_page = session.get(url)
            pywikibot.output(url)
            metadata['url'] = url

            metadata['collectionqid'] = 'Q1616123'
            metadata['collectionshort'] = 'Scheepvaartmuseum'
            metadata['locationqid'] = 'Q1616123'

            # Searching for paintings
            metadata['instanceofqid'] = 'Q3305213'
            metadata['idpid'] = 'P217'

            inv_regex = '<div class="label">Objectnummer</div>[\s\t\r\n]*<div class="value">([^\<]+)</div>'
            ...
            creator_regex = '<div class="label">Vervaardiger</div>[\s\t\r\n]*<div class="value">\s*<a href="https://collectie\.hetscheepvaartmuseum\.nl/search/detail[^\"]+">([^\<]+)</a>\s*\(schilder\)[^<]*<br></div>'
            ...
            date_regex = '<div class="label">Datering</div>[\s\t\r\n]*<div class="value">\s*([^\<]+)</div>'
            ...
            material_regex = '<a href="https://collectie\.hetscheepvaartmuseum\.nl/search/detail\?database=collect&amp;fieldname=Field_Material&amp;value=[^\"]+">([^\<]+)</a>'
            ...
            # All sorts of sizes, using the "dagmaat"
            ...
            image_regex = 'as="image" href="(https://collectie\.hetscheepvaartmuseum\.nl/Content/GetContent/\?value=[^"]+\.jpg&amp;command=getcontent&amp;server=images)&amp;width=\d+&amp;height=\d+"'
            image_regex2 = 'as="image" href="(https://beeldbank\.hetscheepvaartmuseum\.nl/[^"]+)"'
            image_match = re.search(image_regex, item_page.text)
            image_match2 = re.search(image_regex2, item_page.text)

            image_url = None
            if image_match:
                image_url = html.unescape(image_match.group(1)).replace(' ', '%20')
            elif image_match2:
                image_url = html.unescape(image_match2.group(1)).replace(' ', '%20')
...
def main(*args):
    ...
    paintingGen = get_scheepvaartmuseum_generator()
