...
# -*- coding: utf-8 -*-
"""
Bot to import paintings from Maritiem Museum Rotterdam to Wikidata.

The have Adlib/Axiel running at https://mmr-web.adlibhosting.com/ais6/results
...
def get_maritiem_museum_rotterdam_generator():
    """
    Generator to return Maritiem Museum Rotterdam paintings
    """
    start_url = 'https://mmr-web.adlibhosting.com/ais6/search/detail?database=collect&fieldname=Field_Objectcategory&value=Schilderijen'

    session = requests.Session()
    session.get(start_url)

    base_search_url = 'https://mmr-web.adlibhosting.com/ais6/resultsnavigate/%s'
    ...
    for i in range(1, 42):
        ...
        work_url_regex = '<a title="[^"]+" href="https://mmr-web\.adlibhosting\.com/ais6/Details/collect/(\d+)">'
        matches = re.finditer(work_url_regex, search_page.text)

        for match in matches:
            metadata = {}
            url = 'https://mmr-web.adlibhosting.com/ais6/Details/collect/%s' % (match.group(1),)

            item_page = session.get(url)
            pywikibot.output(url)
            metadata['url'] = url

            metadata['collectionqid'] = 'Q2755458'
            metadata['collectionshort'] = 'Maritiem Museum'
            metadata['locationqid'] = 'Q2755458'

            # Searching for paintings
            metadata['instanceofqid'] = 'Q3305213'
            metadata['idpid'] = 'P217'

            inv_regex = '<span class="label">Objectnummer</span><span class="value">([^<]+)</span></div>'
            ...
            # Sometimes a <br> in the title
            title_regex = '<span class="label">Titel</span><span class="value">([^<]+)<'
            ...
            creator_regex = '<span class="label">Vervaardiger</span><span class="value"><a href="https://mmr-web\.adlibhosting\.com/ais6/search/detail\?database=collect&amp;fieldname=Field_Creator[^"]+">\s*([^<]+)</a>\s*\(schilder(\[persoon\])?\)‎</span>'
            ...
            date_regex = '<span class="label">Datum</span><span class="value">\s*([^<]+)</span>'
            ...
            material_regex = '<a href="https://mmr-web\.adlibhosting\.com/ais6/search/detail\?database=collect&amp;fieldname=Field_Material&amp;value=[^"]+">([^<]+)</a>'
            ...
            simple_2d_regex = '<span class="label">Formaat</span><span class="value"><div>(doek|spieraam|paneel)\s*hoogte:\s*(?P<height>\d+(\.\d+)?)\s*cm\s*<br>(doek|spieraam|paneel)\s*breedte:\s*(?P<width>\d+(\.\d+)?)\s*cm<br>'
            ...
            image_regex = 'href="(https://mmr-web\.adlibhosting\.com/ais6/webapiimages/wwwopac\.ashx\?command=getcontent&amp;server=images&amp;value=[^"]+&amp;imageformat=jpg)" title="'
...
def main(*args):
    ...
    paintingGen = get_maritiem_museum_rotterdam_generator()
