...
def getPaintingGenerator(query=u''):
    ...
    * Loop over http://americanart.si.edu/collections/search/artwork/results/index.cfm?rows=10&q=&page=1&start=0&fq=object_type:%22Paintings%22
    http://americanart.si.edu/collections/search/artwork/results/index.cfm?rows=10&q=&start=10&fq=object_type:%22Paintings%22
    * Grab data from paintings
    '''

    baseurl = u'http://americanart.si.edu/collections/search/artwork/results/index.cfm?rows=%s&q=&page=%s&start=%s&fq=object_type:%%22Paintings%%22' 
    ...
    for i in range(start, end, step):
        searchurl = baseurl % (step, i/step, i)
        ...
        itemregex = u'<div class="artworkDetails">\s*<h3><a href="(?P<shorturl>[^"]+)">(?P<title>[^<]+)</a></h3>\s*<p><span title="date">(?P<date>[^<]+)</span>\s*<span title="artist"><b>(?P<artist>[^<]+)</b>\s*</span>(\s*<span title="artist"><b>[^<]+</b>\s*.+\s*</span>)?\s*<span title="medium">(?P<medium>[^<]+)</span>\s*(<span title="dimensions">(?P<dimensions>[^<]+)</span>)?\s*<span title="owner">(?P<owner>[^<]+)</span>\s*<span title="accession number">(?P<id>[^<]+)</span>\s*<span title="location">'
       
        for match in re.finditer(itemregex, searchData, flags=re.M):
            n = n + 1
            metadata = {}
            url = u'http://americanart.si.edu%s' % (match.group('shorturl'),)
            metadata['url'] = url
            title = htmlparser.unescape(unicode(match.group('title'), "utf-8"))
            if len(title) > 220:
                metadata['title'] = title[0:220]
            else:
                metadata['title'] = title
                
            metadata['date'] = htmlparser.unescape(unicode(match.group('date'), "utf-8"))
            artist = htmlparser.unescape(unicode(match.group('artist'), "utf-8"))

            creatorregex = u'^([^,]+),.+$'
            creatormatch = re.match(creatorregex, artist)
            if creatormatch:
                metadata['creator'] = creatormatch.group(1)
            else:
                metadata['creator'] = artist
            
            metadata['medium'] = htmlparser.unescape(unicode(match.group('medium'), "utf-8"))
            if match.group('dimensions'):
                metadata['dimensions'] = htmlparser.unescape(unicode(match.group('dimensions'), "utf-8"))
            metadata['owner'] = htmlparser.unescape(unicode(match.group('owner'), "utf-8"))
            metadata['id'] = htmlparser.unescape(unicode(match.group('id'), "utf-8"))
            ...
            """    
            metadata[u'location'] = u'Q1192305'
            metadata[u'collectionshort'] = u'American Art'
            
            yield metadata
            
        print u'Excepted %s items, got %s items' % (i -start + step, n)
...
def main():
    ...
    paintingsBot = PaintingsBot(paintingGen, 217, u'1192305')
    paintingsBot.run()
