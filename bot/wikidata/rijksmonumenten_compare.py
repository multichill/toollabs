...
class RijksmonumentenCompareBot:
    ...
    def __init__(self):
        ...
        self.rijksmonumenten_wikidata = self.rijksmonumenten_from_wikidata()
        self.former_rijksmonumenten_wikidata = self.former_rijksmonumenten_from_wikidata()
        self.rijksmonumenten_rce = self.rijksmonumenten_from_rce()
        self.former_rijksmonumenten_rce = self.former_rijksmonumenten_from_rce()

    def rijksmonumenten_from_wikidata(self):
        """
    ...
    def former_rijksmonumenten_from_wikidata(self):
        """
        Just return all the usage of Rijksmonument ID as a dict
        :return: Dict
        """
    ...
    def rijksmonumenten_from_rce(self):
        """
        Just return all the usage of Rijksmonument ID as a dict
        :return: Dict
        """
    ...
    def former_rijksmonumenten_from_rce(self):
        """
        Just return all the usage of Rijksmonument ID as a dict
        :return: Dict
        """
...
SELECT DISTINCT ?item ?id WHERE {
  ...
  ?item ceo:heeftJuridischeStatus <https://data.cultureelerfgoed.nl/term/id/rn/3e79bb7c-b459-4998-a9ed-78d91d069227> #}
    ...
    def rijksmonumentComplexOnWikidata(self):
        '''
  ...
  ?uri <https://linkeddata.cultureelerfgoed.nl/def/ceo#complexnummer> ?id .
        ...
        endpoint = u'https://linkeddata.cultureelerfgoed.nl/sparql'
        entity_url = u'https://linkeddata.cultureelerfgoed.nl/cho-kennis/'

        sq = pywikibot.data.sparql.SparqlQuery(endpoint=endpoint, entity_url=entity_url)
        queryresult = sq.select(query)

        for resultitem in queryresult:
            result[int(resultitem.get('id'))] = resultitem
        print (result)
        return result
    ...
    def run(self):
        ...
        Starts the robot.
        """
        pywikibot.output('__TOC__\n== Summary ==')
        pywikibot.output('* Wikidata has %s keys' % (len(self.rijksmonumenten_wikidata.keys())),)
        pywikibot.output('* Wikidata has %s deprecated keys' % (len(self.former_rijksmonumenten_wikidata.keys())),)
        pywikibot.output('* RCE has %s keys' % (len(self.rijksmonumenten_rce.keys())),)
        pywikibot.output('* RCE has %s no Rijksmonument keys' % (len(self.former_rijksmonumenten_rce.keys())),)

        both_sets = set(self.rijksmonumenten_wikidata.keys()) & set(self.rijksmonumenten_rce.keys())
        pywikibot.output('* RCE and Wikidata have %s shared keys' % (len(both_sets), ))

        both_former_sets = set(self.former_rijksmonumenten_wikidata.keys()) & set(self.former_rijksmonumenten_rce.keys())
        pywikibot.output('* RCE and Wikidata have %s shared former keys' % (len(both_former_sets), ))
        ...
        pywikibot.output('* Wikidata and not RCE %s keys' % (len(wikidata_not_rce), ))
        pywikibot.output('* Wikidata and former RCE %s keys' % (len(wikidata_former_rce), ))
        pywikibot.output('* RCE and not Wikidata %s keys' % (len(rce_not_wikidata), ))

        pywikibot.output('== Wikidata former RCE ==\nShould be deprecated')
        for rm_id in sorted(wikidata_former_rce):
            pywikibot.output('* %s - {{Q|%s}}' % (rm_id, self.rijksmonumenten_wikidata.get(rm_id)))

        pywikibot.output('== Wikidata not RCE ==\nShould be figured out')
        for rm_id in sorted(wikidata_all_not_rce):
            pywikibot.output('* %s - {{Q|%s}}' % (rm_id, self.rijksmonumenten_wikidata.get(rm_id)))

        cultureelerfgoed_url = 'https://monumentenregister.cultureelerfgoed.nl/monumenten/'

        pywikibot.output('== RCE not Wikidata ==')
        for rm_id in sorted(rce_not_wikidata):
            pywikibot.output('* %s - %s%s / %s%s' % (rm_id,
                                                     self.rce_entity_url,
        ...
        pywikibot.output('== RCE former Rijksmonument not Wikidata ==')
        for rm_id in sorted(missing_no_rijksmonument):
            pywikibot.output('* %s - %s%s ' % (rm_id, self.rce_entity_url, self.former_rijksmonumenten_rce.get(rm_id), ))
        return
        ...
        for itempage in self.generator:
            ...
            if u'P359' in claims and u'P7135' not in claims and u'P1435' in claims:
                self.fixComplex(itempage)
    ...
    def fixComplex(self, itempage):
        """
        Move the ID from the old property to the new property
        :param itempage:
        :return:
        """
        data = itempage.get()
        ...
        if len(claims.get(u'P1435'))!=1 or len(claims.get(u'P359'))!=1:
            pywikibot.output(u'Weird claim length, skipping')
            return

        current_id = claims.get(u'P359')[0].getTarget()

        if int(current_id) not in self.rijksmonumentComplexRCE:
            pywikibot.output(u'ID not found in RCE complex list, skipping')
            return
        print (current_id)

        summary = u'Moving Rijksmonument complex ID from old to new property'

        newclaim = pywikibot.Claim(self.repo, u'P7135')
        newclaim.setTarget(current_id)
        pywikibot.output('Adding moving id claim on %s' % itempage)
        itempage.addClaim(newclaim, summary=summary)
        itempage.removeClaims(claims.get(u'P359')[0], summary=summary)
    ...
    def addCountry(self, itempage, rkdartistsdocs, refurl):
        ...
        nationality = { 92956 : u'Q29999',} # Dutch -> Kingdom of the Netherlands
        data = itempage.get()
        claims = data.get('claims')
        #dateofbirth = False
        #dateofdeath = False
