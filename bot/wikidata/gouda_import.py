...
# -*- coding: utf-8 -*-
"""
Bot to import paintings from the Museum Gouda to Wikidata.

The have Adlib/Axiel running at https://collectie.museumgouda.nl/results
...
def get_gouda_generator():
    """
    Generator to return Museum Gouda paintings
    """
    start_url = 'https://collectie.museumgouda.nl/search/detail?database=collect&fieldname=Field_Objectname&value=schilderij'

    # TODO: Find missing ones
    session = requests.Session()
    session.get(start_url)

    base_search_url = 'https://collectie.museumgouda.nl/resultsnavigate/%s'
    ...
    for i in range(1, 63):
        ...
        work_url_regex = '<a href="https://collectie.museumgouda.nl/Details/collect/(\d+)">'
        matches = re.finditer(work_url_regex, search_page.text)

        for match in matches:
            metadata = {}
            url = 'https://collectie.museumgouda.nl/Details/collect/%s' % (match.group(1),)

            item_page = session.get(url)
            pywikibot.output(url)
            metadata['url'] = url

            metadata['collectionqid'] = 'Q4360916'
            metadata['collectionshort'] = 'Gouda'
            metadata['locationqid'] = 'Q4360916'
            ...
            date_regex = '<div class="label">Datum</div><div class="value"><p>\s*([^\<]+)</p></div>'
            ...
            material_regex = '<a href="https://collectie\.museumgouda\.nl/search/detail\?database=collect&amp;fieldname=Field_Material&amp;value=[^\"]+">([^\<]+)</a>'
            ...
            if image_match:
                ...
                if not recent_inception:
                    ...
                    metadata['imageoperatedby'] = 'Q4360916'
...
def main(*args):
    ...
    paintingGen = get_gouda_generator()
