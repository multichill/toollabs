...
# -*- coding: utf-8 -*-
"""
Bot to import paintings from the Minneapolis Institute of Arts.

Dump at https://github.com/artsmia/collection all CC0!!!!
"""
import json
import pywikibot
...
class PaintingsBot:
    ...
    def fillCache(self, collectionid, propertyId, queryoverride=u'', cacheMaxAge=1000):
        '''
    ...
    def fixDescription(self, paintingItem, creatorName):
        '''
        ARTSMIA API was messed up and all special chars got messed up. Trying to fix :-(
        '''
        descriptions = paintingItem.get().get('descriptions')
        description_en = descriptions.get('en')
        description_nl = descriptions.get('nl')

        description_en_new = u'painting by %s' % (creatorName,)
        description_nl_new = u'schilderij van %s' % (creatorName,)

        if description_en==description_en_new:
            return

        try:
            description_en_new.encode('ascii')
            pywikibot.output(u'Old description "%s", new description "%s", not replace' % (description_en, description_en_new))
        except UnicodeEncodeError:
            # We want this!
            descriptions['en'] = description_en_new
            descriptions['nl'] = description_nl_new
            summary = u'Fixing messed up unicode ARTSMIA descriptions'
...
def getPaintingGenerator(query=u''):
    '''
    The Minneapolis Institute of Arts provides a dump at https://github.com/artsmia/collection and they have an api.
    The api is located at http://api.artsmia.org/objects/730/full/json (replace 730 with the id).

    Need to build two generators:
    1. Just get all the id's at https://github.com/artsmia/collection/blob/master/departments/6.json and add those
    2. Loop over all 123514 items and check for suitable medium. This sucks
    ...
    #idgenerator = range(1, 123515)
    idgenerator = paintinglistDataObject.get('artworks')
    ...
    for i in idgenerator:
        ...
        if not foundit:
            continue
        metadata = {}
        metadata['collection'] = u'Q1700481'
        metadata['collectionshort'] = u'ARTSMIA'
        metadata['location'] = u'Q1700481'
        
        apiurl = u'http://api.artsmia.org/objects/%s/full/json' % (i,)
        url = u'https://collections.artsmia.org/index.php?page=detail&id=%s' % (i,)
        metadata['url'] = url
        
        itemPage = urllib2.urlopen(apiurl)
        itemData = itemPage.read()
        itemDataObject = json.loads(itemData)

        metadata['title'] = itemDataObject['title']
        metadata['medium'] = itemDataObject['medium']
        metadata['date'] = itemDataObject['dated']
        metadata['creator'] = itemDataObject['artist']
        metadata['id'] = itemDataObject['accession_number']

        if itemDataObject['image']==u'valid' and itemDataObject['image_copyright']==u'' and itemDataObject['restricted']==0:
            imagesize = max(itemDataObject['image_width'], itemDataObject['image_height'])
            metadata['imageurl'] = 'https://api.artsmia.org/images/%s/%s/large.jpg' % (i, imagesize)
            metadata['creditline'] = itemDataObject['creditline']
            metadata['dimension'] = itemDataObject['dimension']
            metadata['description'] = itemDataObject['description']
            metadata['provenance'] = itemDataObject['provenance']
...
def main():
    ...
    paintingsBot = PaintingsBot(paintingGen, 217, u'1700481')
    paintingsBot.run()
