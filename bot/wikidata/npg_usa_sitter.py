...
# -*- coding: utf-8 -*-
"""
Quick and dirty script to add sitters to https://www.wikidata.org/wiki/Wikidata:WikiProject_sum_of_all_paintings/Collection/National_Portrait_Gallery,_Washington,_D.C.
...
def paintingGenerator():
  ...
  ?item p:P195/ps:P195 wd:Q1967614 .
  ...
  BIND(REPLACE(?npgid, '^(.+)$',?formatterurl) AS ?npgurl)  .
...
def addSitter(paintinginfo):
    ...
    deathsitterregex = u'\<dt\>Sitter\<\/dt\>[\s\t\r\n]*\<dd\>[^\<]*(\d\d\d\d)[^\<]*(\d\d\d\d)[^\<]*\<\/dd\>'
    livingsitterregex = u'\<dt\>Sitter\<\/dt\>[\s\t\r\n]*\<dd\>[^\<]*(\d\d\d\d)[^\<]*\<\/dd\>'
