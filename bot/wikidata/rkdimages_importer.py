...
class RKDImagesImporterBot:
    ...
    def run(self):
        ...
        for itempage in self.generator:
            ...
            print rkdimagesid
            rkdimagesurl = u'https://api.rkd.nl/api/record/images/%s?format=json' % (rkdimagesid,)
            refurl = u'https://rkd.nl/explore/images/%s' % (rkdimagesid,)

            # Do some checking if it actually exists?
            rkdimagesPage = requests.get(rkdimagesurl, verify=False)
            rkdimagesJson = rkdimagesPage.json()

            if rkdimagesJson.get('content') and rkdimagesJson.get('content').get('message'):
                pywikibot.output(u'Something went wrong, got "%s" for %s, skipping' % (rkdimagesJson.get('content').get('message'),
                                                                                       rkdimagesid))
                continue

            rkdimagesdocs = rkdimagesJson.get(u'response').get(u'docs')[0]

            if u'P1257' not in claims:
                self.addDepictsIconclass(itempage, rkdimagesdocs, refurl)
    ...
    def addDepictsIconclass(self, itempage, rkdimagesdocs, refurl):
        '''
        Add the worklocation(s) to the itempage.
        Only work on items for which all the locations can be added
        :param itempage: The ItemPage to update
        :param rkdimagesdocs: The json with the RKD information
        :param refurl: The url to add as reference
        :return: Nothing, update the itempage in place
        '''
        if rkdimagesdocs.get('iconclass'):
            for iconclass in rkdimagesdocs.get('iconclass'):
                iconclass_code = iconclass.get('iconclass_code')
                iconclass_text = iconclass.get('iconclass_tekst')
                newclaim = pywikibot.Claim(self.repo, u'P1257')
                newclaim.setTarget(iconclass_code)
                itempage.addClaim(newclaim, summary=iconclass_text)
                self.addReference(itempage, newclaim, refurl)
    ...
    def addReference(self, item, newclaim, url):
        ...
        rkdimagessitem = pywikibot.ItemPage(self.repo,u'Q17299580')
        statedin.setTarget(rkdimagessitem)
