...
# -*- coding: utf-8 -*-
"""
Bot to import paintings from the Rubenshuis to Wikidata.

The have Adlib/Axiel running at https://search.rubenshuis.be/results
...
def get_rubenshuis_generator():
    """
    Generator to return Rubenshuis paintings
    """
    start_url = 'https://search.rubenshuis.be/search/detail?database=collect&fieldname=Field_Objectname&value=schilderij'
    #start_url = 'https://search.rubenshuis.be/search/detail?database=collect&fieldname=Field_Objectname&value=olieverfschets'
    ...
    base_search_url = 'https://search.rubenshuis.be/resultsnavigate/%s'
    ...
    for i in range(1, 10):
        ...
        for match in matches:
            metadata = {}
            url = 'https://search.rubenshuis.be/Details/collect/%s' % (match.group(1),)

            item_page = session.get(url)
            pywikibot.output(url)
            metadata['url'] = url

            metadata['collectionqid'] = 'Q775644'
            metadata['collectionshort'] = 'Rubenshuis'
            metadata['locationqid'] = 'Q775644'
            ...
            if object_name not in ['schilderij', 'triptiek', 'miniatuur [schildering]', 'olieverfschets']:
                print('%s is nota valid object name' % (object_name,))
                continue
            ...
            if dams_id_match:
                ...
                print(json.dumps(dams_json, indent=4))
                canvas = dams_json.get('sequences')[0].get('canvases')[0]
                if canvas.get('rendering'):
                    rendering = canvas.get('rendering')[0]
...
def main(*args):
    dictGen = get_rubenshuis_generator()
    dryrun = False
    create = False
